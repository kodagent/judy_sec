...
from helpers.optimizer_utils import (
    get_cover_letter_instruction,
...
def get_document_file_by_id(document_id):
    ...
    try:
        with connections["giddaa_db"].cursor() as cursor:
            ...
            cursor.execute(
                'SELECT "Id", "Name", "Description", "Extension", "Document", "ExtraProperties" FROM "public"."Documents" WHERE "Id" = %s',
                [document_id],
            )
...
def analyze_impact(text):
    instruction = (
        "Identify and evaluate quantifiable achievements in the following text."
    )
...
def main(resume_content, job_description):
    ...
    for section_name, section_content in resume_section_dict.items():
        section_feedback[section_name] = get_section_feedback(
    ...
    resume_feedback = provide_feedback(
        matching_keywords,
        missing_keywords,
        complexity_score,
        readability_score,
    ...
    optimized_content = optimize_resume(
        resume_content, job_description, resume_feedback
    )
    logger.info("----------------------- FULL RESUME FEEDBACK -----------------------")
    ...
    # 3. Perform analyses
    logger.info(f"----------------------- ANALYSIS -----------------------")
    matching_keywords, matching_score = keyword_analysis(
        optimized_content, job_keywords
    )
    logger.info("------------ MATCHING KEYWORDS ------------")
    logger.info(f"{matching_keywords}")
    logger.info("------------ MATCHING SCORE ------------")
